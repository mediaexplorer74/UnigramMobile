...
namespace Unigram.ViewModels.Settings
{
    public class SettingsBlockedChatsViewModel : TLViewModelBase, IDelegable<IFileDelegate>, IHandle<UpdateFile>
    {
        ...
        {
            Chat selected = await SharePopup.PickChatAsync(Strings.Resources.BlockUser) as Chat;
            if (selected == null)
            {
                return;
            }
            ...
            if (selected.Type is ChatTypePrivate privata)
            {
                ...
                ProtoService.Send(new ToggleMessageSenderIsBlocked(new MessageSenderUser(privata.UserId), 
                    true));
            }
            else
            {
                Items.Insert(0, new MessageSenderChat(selected.Id));
                ProtoService.Send(new ToggleMessageSenderIsBlocked(new MessageSenderChat(selected.Id), 
                    true));
            }
        }
        ...
        {
            ContentDialogResult confirm = await MessagePopup.ShowAsync(
        ...
        public class ItemsCollection : MvxObservableCollection<MessageSender>, ISupportIncrementalLoading
        {
            ...
            public IAsyncOperation<LoadMoreItemsResult> LoadMoreItemsAsync(uint count)
            {
                ...
                {
                    BaseObject response = await _protoService.SendAsync(new GetBlockedMessageSenders(default,
                        Count, 20));
