...
namespace Unigram.ViewModels.Settings
{
    public abstract class SettingsStickersViewModelBase : TLViewModelBase, IHandle<UpdateInstalledStickerSets>, IHandle<UpdateTrendingStickerSets>, IHandle<UpdateRecentStickers>
    {
            ...
            IDictionary<string, object> state)
        {
            ...
            ProtoService.Send(new GetInstalledStickerSets(/*_masks*/default), result =>
            {
                if (result is StickerSets stickerSets)
                {
                    ProtoService.Send(new GetRecentStickers(_masks), resultRecent =>
                    {
                        ...
                        {
                            BeginOnUIThread(() => Items.ReplaceWith(new[] { 
                                new StickerSetInfo(default, default, Strings.Resources.RecentStickers, 
                                default,default,/*"tg/recentlyUsed"*/default, 
                                /*new ClosedVectorPath[0]*/default, false, 
                                default, default, default, false, default,recents.StickersValue.Count,
                                recents.StickersValue) }.Union(stickerSets.Sets)));
                        }
                        else
                        {
            ...
            ProtoService.Send(new GetArchivedStickerSets(/*_masks*/default, 0, 1), result =>
            {
                if (result is StickerSets stickerSets)
                {
        ...
        public override Task OnNavigatedFromAsync(IDictionary<string, object> pageState, bool suspending)
        {
            ...
            {
                _needReorder = false;
                ProtoService.Send(new ReorderInstalledStickerSets(/*_masks*/default, _newOrder));
            ...
            ProtoService.Send(new GetInstalledStickerSets(/*_masks*/default), result =>
            {
                if (result is StickerSets stickerSets)
                {
                    ProtoService.Send(new GetRecentStickers(_masks), resultRecent =>
                    {
                        ...
                        {
                            BeginOnUIThread(() => Items.ReplaceWith(new[] { 
                                new StickerSetInfo(0, Strings.Resources.RecentStickers, 
                                "tg/recentlyUsed", null, new ClosedVectorPath[0],
                                false, false, false, null, default, default,default,default,
                                recents.StickersValue.Count,recents.StickersValue) }.Union(stickerSets.Sets)));
                        }
                        else
                        {
                            BeginOnUIThread(() => Items.ReplaceWith(stickerSets.Sets));
            ...
            ProtoService.Send(new GetInstalledStickerSets(/*_masks*/default), result =>
            {
                if (result is StickerSets stickerSets)
                {
                    ProtoService.Send(new GetRecentStickers(_masks), resultRecent =>
                    {
                        if (resultRecent is Stickers recents && recents.StickersValue.Count > 0)
                        {
                            BeginOnUIThread(() => Items.ReplaceWith(new[] { new StickerSetInfo(
                                0, Strings.Resources.RecentStickers, "tg/recentlyUsed", 
                                null, new ClosedVectorPath[0], 
                                false, false, false, null, default, default,default,default,
                                recents.StickersValue.Count, recents.StickersValue) }.Union(
                                stickerSets.Sets)));
                        }
                        else
                        {
