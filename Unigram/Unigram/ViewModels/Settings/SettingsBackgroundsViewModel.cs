...
namespace Unigram.ViewModels.Settings
{
    public class SettingsBackgroundsViewModel : TLViewModelBase
    {
        ...
        private void RefreshItems()
        {
            ProtoService.Send(new GetBackgrounds(Settings.Appearance.IsDarkTheme()), result =>
            {
                if (result is Backgrounds wallpapers)
                {
                    ...
                    //{
                    //    //items.Insert(0, selected = new Background(Constants.WallpaperLocalId,
                    //       new PhotoSize[0], 0));
                    //}
        ...
        private async void LocalExecute()
        {
            try
            {
                ...
                if (file != null)
                {
                    ...
                    NavigationService.Navigate(typeof(BackgroundPage), 
                        Constants.WallpaperLocalFileName + $"#{token}");
                }
            }
            catch { }
        }
        ...
        {
            var confirm = await MessagePopup.ShowAsync(Strings.Resources.ResetChatBackgroundsAlert,
                Strings.Resources.ResetChatBackgroundsAlertTitle, Strings.Resources.Reset, 
