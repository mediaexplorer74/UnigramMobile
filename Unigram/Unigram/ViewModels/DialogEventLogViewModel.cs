...
namespace Unigram.ViewModels
{
    public class DialogEventLogViewModel : DialogViewModel
    {
        ...
        private ChatEventLogFilters _filters = new ChatEventLogFilters(
        ...
        public override async Task LoadEventLogSliceAsync(string query = "")
        {
            using (await _loadMoreLock.WaitAsync())
            {
                ...
                if (response is ChatEvents events)
                {
                    ...
                    if (target != null)
                    {
                        /*replied.Insert(0, _messageFactory.Create(this, 
                            new Message(0, target.SenderId, target.ChatId, 
                            null, target.SchedulingState, target.IsOutgoing, 
                            false, false, false, false, true, false, false, false, 
                            false, false, false, false, target.IsChannelPost, 
                            false, target.Date, 0, null, null, null, 0, 0, 0, 0, 0, 0, 
                            string.Empty, 0, string.Empty, new MessageHeaderDate(), null)));*/
                    }
        ...
        private Message CreateMessage(long chatId, bool isChannel, ChatEvent chatEvent, bool child = false)
        {
            ...
            return default;// new Message(chatEvent.Id, sender,
                           // chatId, null, null, false, false, false,
                           // false, false, false, false, false, false,
                           // false, false, false, false, isChannel,
                           // false, chatEvent.Date, 0, null, null,
                           // null, 0, 0, 0, 0, 0, 0, string.Empty, 0, string.Empty, null, null);
        }

        private MessageViewModel GetMessage(long chatId, bool isChannel, 
            ChatEvent chatEvent, bool child = false)
        {
            var message = _messageFactory.Create(this, 
                CreateMessage(chatId, isChannel, chatEvent, child));
        ...
        private MessageContent GetMessageContent(ChatEvent item, bool channel)
        {
            if (item.Action is ChatEventDescriptionChanged descriptionChanged)
            {
                ...
                return new MessageText(text, webPage, default);
            }
            ...
            else if (item.Action is ChatEventPermissionsChanged permissionChanged)
            {
                ...
                ChatPermissions o = permissionChanged.OldPermissions as ChatPermissions;
                ChatPermissions n = permissionChanged.NewPermissions as ChatPermissions;
                ...
                return new MessageText(new FormattedText(text, entities), null,default);
            }
            else if (item.Action is ChatEventMemberRestricted memberRestricted)
            {
                ...
                {
                    o = new ChatMemberStatusRestricted(false, oldBanned.BannedUntilDate, 
                        new ChatPermissions(false, false, false, false, false, false, false, false,
                        default,default,default,default,default,default));
                }
                else if (memberRestricted.OldStatus is ChatMemberStatusMember)
                {
                    o = new ChatMemberStatusRestricted(true, 0, 
                        new ChatPermissions(true, true, true, true, true, true, true, true,
                        default,default,default,default,default,default));
                }
                ...
                {
                    n = new ChatMemberStatusRestricted(false, newBanned.BannedUntilDate, 
                        new ChatPermissions(false, false, false, false, false, false, false, false,
                        default,default,default,default,default,default));
                }
                else if (memberRestricted.NewStatus is ChatMemberStatusMember)
                {
                    n = new ChatMemberStatusRestricted(true, 0, 
                        new ChatPermissions(true, true, true, true, true, true, true, true,
                        default,default,default,default,default,default));
                }
                ...
                if (!channel && (n == null || n != null && o != null /*&& n.RestrictedUntilDate != o.RestrictedUntilDate*/))
                {
                    ...
                    rights = new StringBuilder(String.Format(
                        str, GetUserName(whoUser, entities, str.IndexOf("{0}")), bannedDuration));
                    var added = false;
                    if (o == null)
                    {
                        o = new ChatMemberStatusRestricted(true, 0, 
            ...
            else if (item.Action is ChatEventMemberPromoted memberPromoted)
            {
                ...
                {
                    return new MessageText(new FormattedText(builder.ToString(), entities), 
                        null, default);
                }
                ...
                */
                ...
                return new MessageText(new FormattedText(builder.ToString(), entities), null, default);
            }
            ...
            {
                if (messageEdited.NewMessage.Content is MessageText editedText 
                    && messageEdited.OldMessage.Content is MessageText oldText)
                {
        ...
        private string GetUserName(BaseObject sender, List<TextEntity> entities, int offset)
        {
            if (sender is User user)
            {
                ...
                if (offset >= 0)
                {
                    entities.Add(new TextEntity(offset, name.Length, 
                        new TextEntityTypeMentionName(user.Id)));
                }
                ...
                if (offset >= 0)
                {
                    entities.Add(new TextEntity(name.Length + offset + 2, user.FirstName.Length + 1, 
                        new TextEntityTypeMentionName(user.Id)));
                }

                return string.Format("{0} (@{1})", name, user.FirstName);
            }
