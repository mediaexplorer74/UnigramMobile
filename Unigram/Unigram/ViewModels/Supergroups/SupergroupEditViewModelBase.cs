        ...
        IHandle<UpdateBasicGroupFullInfo>
    {
        ...
        protected async void LoadAdminedPublicChannels()
        {
            ...
            if (response is Telegram.Td.Api.Chats chats)
            {
                ...
                {
                    var chat = ProtoService.GetChat(id) as Chat;
                    if (chat != null)
                    {
                        result.Add(chat);
                    }
        ...
        private async void RevokeLinkExecute(Chat chat)
        {
            if (chat.Type is ChatTypeSupergroup super)
            {
                ...
                dialog.Message = string.Format(Strings.Resources.RevokeLinkAlert, 
                    MeUrlPrefixConverter.Convert(CacheService, supergroup.Id.ToString(), true), chat.Title);
        ...
        public async void CheckAvailability(string text)
        {
            ...
            var supergroup = CacheService.GetSupergroup(chat) as Supergroup;
            if (supergroup != null && string.Equals(text, supergroup.Id.ToString()))
            {
                IsLoading = false;
                IsAvailable = false;
                ErrorMessage = null;
