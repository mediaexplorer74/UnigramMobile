...
namespace Unigram.ViewModels.Supergroups
{
        ...
        IHandle<UpdateSupergroupFullInfo>
    {
        ...
        public void Handle(UpdateSupergroupFullInfo update)
        {
            ...
            {
                BeginOnUIThread(() => UpdateSupergroupFullInfo(chat, 
                    ProtoService.GetSupergroup(update.SupergroupId), update.SupergroupFullInfo));
            }
        }
        ...
        private async void LinkExecute(Chat linkedChat)
        {
            ...
            else
            {
                ...
                if (CacheService.TryGetSupergroup(linkedChat, out Supergroup linkedSupergroup))
                {
                    ...
                    if (string.IsNullOrEmpty(linkedSupergroup.Id.ToString()))
                    {
                        message = string.Format(
                            Strings.Resources.DiscussionLinkGroupPublicPrivateAlert, 
                            linkedChat.Title, chat.Title);
                    }
                    else
                    {
                        if (string.IsNullOrEmpty(supergroup.Id.ToString()))
                        {
