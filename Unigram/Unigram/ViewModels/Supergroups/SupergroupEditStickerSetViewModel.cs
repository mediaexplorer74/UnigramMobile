...
namespace Unigram.ViewModels.Supergroups
{
    public class SupergroupEditStickerSetViewModel : TLViewModelBase, IHandle<UpdateSupergroupFullInfo>
    {
        ...
        {
            ProtoService.Send(new GetInstalledStickerSets(/*false*/default), result =>
            {
        ...
        private async void UpdateSupergroupFullInfo(Chat chat, Supergroup group, SupergroupFullInfo fullInfo)
        {
            ...
            else
            {
                ...
                {
                    SelectedItem = new StickerSetInfo(set.Id, set.Title, 
                        set.Name, set.Thumbnail, set.ThumbnailOutline, set.IsInstalled, 
                        set.IsArchived, set.IsOfficial, /*set.StickerType*/default,
                        /*set.IsViewed*/default,
                        default,default, default,
                        set.Stickers.Count, set.Stickers);
                    ShortName = set.Name;
                }
            }
        }
        ...
        private async Task CheckAvailabilityAsync(string shortName)
        {
            ...
            if (response is StickerSet stickerSet)
            {
                IsLoading = false;
                IsAvailable = true;
                SelectedItem = new StickerSetInfo
                    (stickerSet.Id, stickerSet.Title, stickerSet.Name,
                    stickerSet.Thumbnail, stickerSet.ThumbnailOutline, 
                    stickerSet.IsInstalled, stickerSet.IsArchived, 
                    stickerSet.IsOfficial, /*stickerSet.StickerType*/default, 
                    /*stickerSet.IsViewed*/default, default,default,default,
                    stickerSet.Stickers.Count, stickerSet.Stickers);
