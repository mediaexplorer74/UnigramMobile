...
namespace Unigram.ViewModels.Supergroups
{
    public class SupergroupEditRestrictedViewModel : TLViewModelBase, IDelegable<IMemberDelegate>
    {
            ...
            IDictionary<string, object> state)
        {
            ...
            var response = await ProtoService.SendAsync(new GetChatMember(chat.Id, 
                new MessageSenderUser(userId)));
        ...
        private async void DismissExecute()
        {
            ...
            var response = await ProtoService.SendAsync(new SetChatMemberStatus(chat.Id, 
                member.MemberId, new ChatMemberStatusMember()));
            if (response is Ok)
            {
                NavigationService.GoBack();
                NavigationService.Frame.ForwardStack.Clear();
