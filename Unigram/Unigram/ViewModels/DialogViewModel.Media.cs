...
namespace Unigram.ViewModels
{
    public partial class DialogViewModel
    {
        ...
        public async void AnimationSendExecute(Animation animation, bool? schedule, bool? silent)
        {
            ...
            var reply = GetReply(true);
            InputMessageAnimation input = default;//new InputMessageAnimation(
                //new InputFileId(animation.AnimationValue.Id), animation.Thumbnail?.ToInput(), new int[0], animation.Duration, animation.Width, animation.Height, null);
        ...
        private async Task SendStorageMediaAsync(Chat chat, StorageMedia storage, FormattedText caption, bool asFile, MessageSendOptions options)
        {
            ...
            {
                await SendDocumentAsync(chat, storage.File, caption ?? storage.Caption, options);
            }
            else if (storage is StoragePhoto photo)
            {
                await SendPhotoAsync(chat, storage.File, caption ?? storage.Caption, asFile, storage.Ttl, storage.IsEdited ? storage.EditState : null, options);
            }
            else if (storage is StorageVideo video)
            {
                await SendVideoAsync(chat, storage.File, caption ?? storage.Caption, video.IsMuted, asFile, storage.Ttl, await video.GetEncodingAsync(), video.GetTransform(), options);
            }
        }
        ...
        public async Task<MessageSendOptions> PickMessageSendOptionsAsync(bool? schedule = null, bool? silent = null)
        {
            ...
            if (schedule == true || (_type == DialogType.ScheduledMessages && schedule == null))
            {
                ...
                var dialog = new ScheduleMessagePopup(user, 
                    DateTime.Now.ToTimestamp(), CacheService.IsSavedMessages(chat));
                var confirm = await dialog.ShowQueuedAsync();
                ...
                {
                    return default;//new MessageSendOptions(false, false, false, new MessageSchedulingStateSendWhenOnline());
                }
                else
                {
                    return default;//new MessageSendOptions(false, false, false, new MessageSchedulingStateSendAtDate(dialog.Value.ToTimestamp()));
                }
            }
            else
            {
                return default;//new MessageSendOptions(silent ?? false, false, false, null);
            }
        }

        private async Task<BaseObject> SendMessageAsync(Chat chat, long replyToMessageId, 
            InputMessageContent inputMessageContent, MessageSendOptions options)
        {
            if (options == null)
            {
            ...
            var response = await ProtoService.SendAsync(
                new SendMessage(chat.Id, _threadId, /*replyToMessageId*/default, options, null, 
                inputMessageContent));
            if (response is Error error)
            {
                ...
                {
                    await MessagePopup.ShowAsync(Strings.Resources.MessageScheduledLimitReached, 
                        Strings.Resources.AppName, Strings.Resources.OK);
                }
            }
        ...
        private async Task<BaseObject> EditMessageAsync(MessageViewModel message, 
            InputFile inputFile, FileType type, Func<InputFile, InputMessageContent> inputMessageContent)
        {
            var response = await ProtoService.SendAsync(new UploadFile(inputFile, type, 32));
            if (response is Telegram.Td.Api.File file)
            {
                ComposerHeader = new MessageComposerHeader { EditingMessage = message,
                    EditingMessageMedia = null, EditingMessageFileId = file.Id };
            }
        ...
        private async Task SendPollAsync(bool forceQuiz, bool forceRegular, bool forceAnonymous)
        {
            ...
            var reply = GetReply(true);
            var input = new InputMessagePoll(dialog.Question, dialog.Options, dialog.IsAnonymous, 
                dialog.Type, 0, 0, false);
        ...
        private async Task<BaseObject> SendGroupedAsync(ICollection<StorageMedia> items, 
            FormattedText caption, MessageSendOptions options)
        {
            var chat = _chat;
            if (chat == null)
            {
            ...
            foreach (var item in items)
            {
                ...
                {
                    var factory = await _messageFactory.CreatePhotoAsync(photo.File, false, 
                        photo.Ttl, photo.IsEdited ? photo.EditState : null);
                    if (factory != null)
                    {
                ...
                {
                    var factory = await _messageFactory.CreateVideoAsync(video.File, 
                        video.IsMuted, false, video.Ttl, await video.GetEncodingAsync(), 
                        video.GetTransform());
                    if (factory != null)
                    {
                        var input = factory.Delegate(factory.InputFile, firstCaption ?? item.Caption);
            ...
            return await ProtoService.SendAsync(new SendMessageAlbum(chat.Id, _threadId,
                /*reply*/default,
                options, operations));
        }
        ...
        private FormattedText GetFormattedText(string text)
        {
            ...
            text = text.Format();
            return Client.Execute(new ParseMarkdown(
                new FormattedText(text, new TextEntity[0]))) as FormattedText;
        }
        ...
        public async Task HandlePackageAsync(DataPackageView package)
        {
            try
            {
                if (package.AvailableFormats.Contains(StandardDataFormats.Bitmap))
                {
                    ...
                    var fileName = string.Format("image_{0:yyyy}-{0:MM}-{0:dd}_{0:HH}-{0:mm}-{0:ss}.png",
                        DateTime.Now);
                    var cache = await ApplicationData.Current.TemporaryFolder.CreateFileAsync(fileName, 
                        CreationCollisionOption.GenerateUniqueName);

                    using (var stream = await bitmap.OpenReadAsync())
                    {
                        var result = await ImageHelper.TranscodeAsync(stream, cache, 
                ...
                else if (package.AvailableFormats.Contains(StandardDataFormats.Text))
                {
                    ...
                    field.Document.GetRange(field.Document.Selection.EndPosition, 
                        field.Document.Selection.EndPosition).SetText(TextSetOptions.None, text);
                }
