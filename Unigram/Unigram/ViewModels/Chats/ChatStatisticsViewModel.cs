            ...
            NavigationMode mode, IDictionary<string, object> state)
        {
            ...
            if (response is ChatStatistics statistics)
            {
                ...
                if (statistics is ChatStatisticsChannel channelStats)
                {
                    ...
                    /* await ProtoService.SendAsync(
                        new GetMessages(chatId, 
                        channelStats.RecentMessageInteractions.Select(x => x.MessageId).ToArray())) 
                    as Messages; */
                    if (messages == null)
                    {
                        return;
                    ...
                    foreach (var message in messages.MessagesValue)
                    {
                        ...
                        ChatStatisticsChannel counters = default;
                        /* channelStats.RecentMessageInteractions.FirstOrDefault(
                         x => x.MessageId == message.Id);
                        interactions.Add(new MessageInteractionCounters(
                        message, counters.ForwardCount, counters.ViewCount));*/
                    }
