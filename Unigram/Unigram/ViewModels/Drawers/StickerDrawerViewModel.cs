...
namespace Unigram.ViewModels.Drawers
{
    public class StickerDrawerViewModel : TLViewModelBase, IHandle<UpdateRecentStickers>, IHandle<UpdateFavoriteStickers>, IHandle<UpdateInstalledStickerSets>
    {
        ...
        public void SyncStickers(Chat chat)
        {
            ...
            ProtoService.Send(new GetFavoriteStickers(), result1 =>
            {
                ...
                {
                    ProtoService.Send(new GetInstalledStickerSets(/*false*/default), result3 =>
                    {
                        if (result1 is Stickers favorite && result2 is Stickers recent 
                        && result3 is StickerSets sets)
                        {
    ...
    public class StickerSetViewModel
    {
        ...
        public bool IsAnimated => false;//_set?.StickerType is StickerTypeAnimated || _set?.StickerType is StickerTypeVideo 
            //|| _info.StickerType is StickerTypeAnimated || _info.StickerType is StickerTypeVideo;
        public bool IsMasks => _set?.StickerType is StickerTypeMask || _info.StickerType is StickerTypeMask;
    ...
    public class StickerViewModel
    {
        ...
        public bool IsAnimated => false;//_sticker?.Type is StickerTypeAnimated || _sticker?.Type is StickerTypeVideo;
        public bool IsMask => false;//_sticker?.Type is StickerTypeMask;
            ...
            return AsyncInfo.Run(async token =>
            {
                if (phase == 0)
                {
                    var response = await _protoService.SendAsync(
                        new SearchInstalledStickerSets(/*_masks*/default, _query, 100));
                    if (response is StickerSets sets)
                    {
                        foreach (var item in sets.Sets.Select(x => new StickerSetViewModel(
                ...
                else if (phase == 1 && _query.Length > 1)
                {
                    ...
                    {
                        BaseObject response = await _protoService.SendAsync(
                            /*new GetStickers(_query, 100)*/default);
                    ...
                    {
                        var emojis = await _protoService.SendAsync(new SearchEmojis(_query, false, new[] 
                        { _inputLanguage })) as Emojis;
                        if (emojis != null)
                        {
                ...
                {
                    BaseObject response = await _protoService.SendAsync(
                        /*new SearchStickerSets(_query)*/default);
                    if (response is StickerSets sets)
                    {
                        foreach (var item in sets.Sets.Select(x => new StickerSetViewModel(
                            _protoService, _aggregator, x, x.Covers)))
                        {
                            Add(item);
                        }
