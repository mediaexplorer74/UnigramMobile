...
namespace Unigram.ViewModels
{
    public partial class DialogViewModel : IMessageDelegate
    {
        ...
        public bool CanBeDownloaded(MessageViewModel message)
        {
            ...
            {
                return Settings.AutoDownload.ShouldDownloadVideo(GetChatType(chat), (int)animation.AnimationValue.Size);
            }
            else if (content is Audio audio)
            {
                return Settings.AutoDownload.ShouldDownloadDocument(GetChatType(chat), (int)audio.AudioValue.Size);
            }
            else if (content is Document document)
            {
                return Settings.AutoDownload.ShouldDownloadDocument(GetChatType(chat), (int)document.DocumentValue.Size);
            }
            ...
            {
                return Settings.AutoDownload.ShouldDownloadVideo(GetChatType(chat), (int)video.VideoValue.Size);
            }
            else if (content is VideoNote videoNote)
            {
                return Settings.AutoDownload.ShouldDownloadVideo(GetChatType(chat), (int)videoNote.Video.Size);
            }
        ...
        public async void OpenLocation(Location location, string title)
        {
            ...
            if (title != null)
            {
                await Windows.System.Launcher.LaunchUriAsync(
                    new Uri(string.Format(CultureInfo.InvariantCulture, "bingmaps:?collection=point.{0}_{1}_{2}", location.Latitude, location.Longitude, WebUtility.UrlEncode(title))), options);
            }
            else
            {
                await Windows.System.Launcher.LaunchUriAsync(
                    new Uri(string.Format(CultureInfo.InvariantCulture, "bingmaps:?collection=point.{0}_{1}", location.Latitude, location.Longitude)), options);
            }
        }
        ...
        public async void OpenUsername(string username)
        {
            ...
            {
                await MessagePopup.ShowAsync(Strings.Resources.NoUsernameFound, 
        ...
        public void OpenViaBot(long viaBotUserId)
        {
            ...
            if (user != null)
            {
                ...
                ResolveInlineBot(user.FirstName);
            }
        }
        ...
        public void OpenChat(long chatId, bool profile = false)
        {
            ...
            {
                //NavigationService.NavigateToChat(/*chat*/default);
            }
        }
        ...
        public void OpenChat(long chatId, long messageId)
        {
            ...
            //NavigationService.NavigateToChat(/*chat*/default, message: messageId);
        }
