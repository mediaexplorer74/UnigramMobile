...
namespace Unigram.ViewModels.Payments
{
    public class PaymentReceiptViewModel : TLViewModelBase
    {
        ...
        public override async Task OnNavigatedToAsync(object parameter, NavigationMode mode, IDictionary<string, object> state)
        {
            ...
            var response = await ProtoService.SendAsync(new GetPaymentReceipt(navigation.ChatId, 
                navigation.ReceiptMessageId));
            if (response is PaymentReceipt receipt)
            {
                Receipt = receipt;
                Bot = ProtoService.GetUser(/*receipt.PaymentsProviderUserId*/receipt.SellerBotUserId) 
                    as User;

                var second = await ProtoService.SendAsync(new GetMessage(navigation.ChatId, 
                    navigation.ReceiptMessageId));
                if (second is Message message1 && message1.Content is MessagePaymentSuccessful payment)
                {
                    Payment = payment;

                    var third = await ProtoService.SendAsync(new GetMessage(navigation.ChatId, 
