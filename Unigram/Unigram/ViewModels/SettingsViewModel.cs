         ...
         IHandle<UpdateFile>
    {
        ...
        private async void EditPhotoExecute(StorageMedia media)
        {
            if (media is StorageVideo)
            {
                ...
                rectangle.Width = Math.Min(conversion.CropRectangle.Width, 
                    conversion.CropRectangle.Height);
                rectangle.Height = rectangle.Width;

                conversion.CropRectangle = rectangle;

                var generated = await media.File.ToGeneratedAsync(
                    ConversionType.Transcode, JsonConvert.SerializeObject(conversion));
                //var response = await ProtoService.SendAsync(
                //    new SetProfilePhoto(new InputChatPhotoAnimation(generated, 0)));
            }
            else
            {
                var generated = await media.File.ToGeneratedAsync(
                    ConversionType.Compress, JsonConvert.SerializeObject(media.EditState));
                //var response = await ProtoService.SendAsync(
                //new SetProfilePhoto(new InputChatPhotoStatic(generated)));
            }
        }
