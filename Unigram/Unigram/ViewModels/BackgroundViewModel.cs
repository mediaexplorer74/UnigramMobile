    ...
    public class BackgroundViewModel : TLViewModelBase, IDelegable<IBackgroundDelegate>
    {
        ...
        public override async Task OnNavigatedToAsync(object parameter, NavigationMode mode, IDictionary<string, object> state)
        {
            ...
            if (_item.Type is BackgroundTypePattern || _item.Type is BackgroundTypeFill)
            {
                ...
                {
                    Patterns.ReplaceWith(new[] { new Background(0, true, false, string.Empty, 
                        null, new BackgroundTypeFill(new BackgroundFillSolid())) }.Union(
                        backgrounds.BackgroundsValue.Where(x => x.Type is BackgroundTypePattern)));
                    SelectedPattern = backgrounds.BackgroundsValue.FirstOrDefault(
                        x => x.Id == background.Id);
                }
            }
        ...
        private async void DoneExecute()
        {
            ...
            if (wallpaper.Id == Constants.WallpaperLocalId && StorageApplicationPermissions.FutureAccessList.ContainsItem(wallpaper.Name))
            {
                ...
                task = ProtoService.SendAsync(new SetBackground(new InputBackgroundLocal(generated), 
                    new BackgroundTypeWallpaper(_isBlurEnabled, _isMotionEnabled), 
                    Settings.Appearance.IsDarkTheme()));
            }
            else
            {
                ...
                {
                    type = new BackgroundTypePattern(GetFill(), Math.Abs(_intensity), _intensity < 0, 
                        _isMotionEnabled);
                }
                ...
                task = ProtoService.SendAsync(new SetBackground(new InputBackgroundRemote(wallpaper.Id), 
                    type, Settings.Appearance.IsDarkTheme()));
            }
