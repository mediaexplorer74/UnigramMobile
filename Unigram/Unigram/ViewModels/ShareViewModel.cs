        ...
        private async void SendExecute()
        {
            ...
            if (!string.IsNullOrEmpty(_comment))
            {
                ...
                foreach (var chat in chats)
                {
                    var response = await ProtoService.SendAsync(
                        new SendMessage(chat.Id, 0, /*0*/default, 
                        new MessageSendOptions(false, false, false, default,default,default,default), 
                        null, 
                        new InputMessageText(formatted, /*false*/default, false)));
                }
            }
            ...
            if (_messages != null)
            {
                foreach (var chat in chats)
                {
                    if (IsWithMyScore)
                    {
                        var response = await ProtoService.SendAsync(new SendMessage(chat.Id, 0, default, 
                            new MessageSendOptions(false, false, false, default,default,default,default), 
                            null, 
                            new InputMessageForwarded(_messages[0].ChatId, _messages[0].Id, 
                            true, new MessageCopyOptions(false, false, null))));
                    }
                    else
                    {
                        var album = false;
                        ...
                        var response = await ProtoService.SendAsync(
                            new ForwardMessages(chat.Id, _messages[0].ChatId,default,
                            _messages.Select(x => x.Id).ToList(), 
                            new MessageSendOptions(false, false, false, default,default,default,default), 
                            _sendAsCopy || _removeCaptions, _removeCaptions));
                    }
                }
            ...
            else if (_inputMedia != null)
            {
                ...
                {
                    var response = await ProtoService.SendAsync(
                        new SendMessage(chat.Id, 0, default, new MessageSendOptions(
                            false, false, false, default,default,default,default), null, _inputMedia));
                }
