...
namespace Unigram.ViewModels
{
    public partial class DialogViewModel : TLViewModelBase, IDelegable<IDialogDelegate>
    {
        ...
        protected async Task AddHeaderAsync()
        {
            ...
            UserFullInfo fullInfo = CacheService.GetUserFull(user.Id);
            if (fullInfo == null)
            {
            ...
            {
                TextEntities result = default;//ProtoService.Execute(new GetTextEntities(fullInfo.Description)) as TextEntities;
                ...
                entities.Add(new TextEntity(0, Strings.Resources.BotInfoTitle.Length, 
                    new TextEntityTypeBold()));

                var message = $"{Strings.Resources.BotInfoTitle}{Environment.NewLine}";
                    // + $"{fullInfo.Description}";
                var text = new FormattedText(message, entities);

                //RnD
                //Items.Insert(0, _messageFactory.Create(this, new Message
                //    (0, new MessageSenderUser(user.Id), 
                //    chat.Id, null, null, false, false, false, false, false, 
                //    true, false, false, false, false, false, false, false, false, false,
                //    0, 0, null, null, null, 0, 0, 0, 0, 0, 0, string.Empty, 0, 
                //    string.Empty, new MessageText(text, null), null)));
            ...
            if (thread != null)
            {
                var replied = thread.Messages.OrderBy(x => x.Id).Select(
                    x => _messageFactory.Create(this, x)).ToList();
                await ProcessMessagesAsync(chat, replied);
               ...
               if (Items.Count > 0)
                {
                    //RnD
                    //Items.Insert(0, _messageFactory.Create(this, 
                    //    new Message(0, previous.SenderId, previous.ChatId, null, null, 
                    //    previous.IsOutgoing, false, false, false, false, true, false, false,
                    //    false, false, false, false, false, previous.IsChannelPost, false, 
                    //    previous.Date, 0, null, null, null, 0, 0, 0, 0, 0, 0, string.Empty, 
                    //    0, string.Empty, new MessageCustomServiceAction(Strings.Resources.DiscussionStarted),
                    //    null)));
                }
                else
                {
                ...
                if (response is Messages messages)
                {
                    ...
                    if (lastReadMessageId != lastMessageId)
                    {
                        ...
                        if (target != null)
                        {
                            ...
                            {
                                Logs.Logger.Debug(Logs.Target.Chat, 
                                    "Looking for first unread message, can't find it");
                            }
        ...
        */
                ...
                if (message.Content is MessageText text)
                {
                    if (Emoji.TryCountEmojis(text.Text.Text, out int count, 3))
                    {
                        ...
                        foreach (var sticker in set.Stickers)
                        {
                            ...
                            {
                                //RnD (isPremium arg)
                                message.GeneratedContent = new MessageSticker(sticker,false);
                                continue;
                            }
                        }
        ...
        private void ProcessFiles(Chat chat, IList<MessageViewModel> messages, MessageViewModel parent = null)
        {
            foreach (var message in messages)
            {
                ...
                if (target.IsSaved() || ((chat.Type is ChatTypeBasicGroup 
                    || chat.Type is ChatTypeSupergroup) && !target.IsOutgoing && !target.IsChannelPost))
                {
                    if (target.IsSaved())
                    {
                        if (target.ForwardInfo?.Origin is MessageForwardOriginUser fromUser)
                        {
                            User user = default;//target.ProtoService.GetUser(fromUser.SenderUserId);
                    ...
                    {
                        if (CacheService.TryGetUser(target.SenderId, out User senderUser)
...
#pragma warning disable CS4014
            ...
            {
               // Logs.Logger.Debug(Logs.Target.Chat, string.Format("{0} - Loadings scheduled messages", 
               //     chat.Id));
            ...
            {
                //Logs.Logger.Debug(Logs.Target.Chat,
                //    string.Format("{0} - Loading messages from specific id", chat.Id));
            ...
            else
            {
                ...
                {
                    bool a = Settings.Chats.TryRemove(/*chat.Id*/default, _threadId, 
                        ChatSetting.ReadInboxMaxId, out v1);
                    bool b = Settings.Chats.TryRemove(/*chat.Id*/default, _threadId, 
                        ChatSetting.Index, out v2);
                    return a && b;
                }
                ...
                {
                    lastReadMessageId = default;//chat.LastReadInboxMessageId;
                    lastMessageId = default;//chat.LastMessage?.Id ?? long.MaxValue;
                }

                if (TryRemove(/*chat.Id*/default, out long readInboxMaxId, out long start) &&
                    readInboxMaxId == lastReadMessageId &&
                    start < lastReadMessageId)
                {
                    if (Settings.Chats.TryRemove(/*chat.Id*/default, _threadId, 
                        ChatSetting.Pixel, out double pixel))
                    {
                        //Logs.Logger.Debug(Logs.Target.Chat, 
                        //    string.Format("{0} - Loading messages from specific pixel", chat.Id));
                    ...
                    {
                        //Logs.Logger.Debug(Logs.Target.Chat, 
                        // string.Format("{0} - Loading messages from specific id, pixel missing", chat.Id));
                ...
                {
                    //Logs.Logger.Debug(Logs.Target.Chat, 
                    //    string.Format("{0} - Loading messages from LastReadInboxMessageId: {1}", chat.Id, chat.LastReadInboxMessageId));
...
#pragma warning restore CS4014
            ...
            if (_type == DialogType.History && App.DataPackages.TryRemove(/*chat.Id*/default, 
                out DataPackageView package))
            {
        ...
        private void ShowSwitchInline(IDictionary<string, object> state)
        {
            if (_type == DialogType.History && state.TryGet("switch_query", out string query) && state.TryGet("switch_bot", out long userId))
            {
                ...
                SetText(string.Format("@{0} {1}", bot.FirstName, query), focus: true);
                ResolveInlineBot(bot.FirstName, query);
            }
        }
        ...
        public void SaveDraft(bool clear = false)
        {
            ...
            if (!string.IsNullOrWhiteSpace(formattedText.Text) || reply != 0)
            {
                ...
                draft = default;//new DraftMessage(reply, 0, new InputMessageText(formattedText, false, false));
            }
        ...
        private void ClearReplyExecute()
        {
            ...
            else
            {
                if (container.EditingMessage != null)
                {
                    ...
                    {
                        ProtoService.Send(/*new CancelUploadFile(fileId)*/default);
                    }
            ...
            if (header?.EditingMessage != null)
            {
                ...
                if (factory != null)
                {
                    var response = await ProtoService.SendAsync(
                        new UploadFile(factory.InputFile, factory.Type, 32));
                    if (response is File file)
                    {
                        if (file.Remote.IsUploadingCompleted)
                        {
                            ComposerHeader = null;
                            ProtoService.Send(new EditMessageMedia(chat.Id, header.EditingMessage.Id,
                                null, header.EditingMessageMedia.Delegate(new InputFileId(file.Id),
                                header.EditingMessageCaption)));
                        }
                        else
                        {
                ...
                else
                {
                    ...
                    {
                        function = new EditMessageText(chat.Id, editing.Id, null, 
                            new InputMessageText(formattedText, /*disablePreview*/default, true));
                    }
                    else
                    {
            ...
            else
            {
                ...
                else
                {
                    if (text.Length > CacheService.Options.MessageTextLengthMax)
                    {
                        ...
                        {
                            var input = new InputMessageText(split, /*disablePreview*/default, true);
                            await SendMessageAsync(chat, reply, input, options);
                        }
                    }
                    else if (text.Length > 0)
                    {
                        var input = new InputMessageText(formattedText, /*disablePreview*/default, true);
                        await SendMessageAsync(chat, reply, input, options);
                    }
                    else
                    {
...
*/
        ...
        private async void ReportSpamExecute(ChatReportReason reason)
        {
            ...
            ProtoService.Send(new ReportChat(chat.Id, new long[0], /*reason*/default, string.Empty));
        ...
        private async void UnblockExecute()
        {
            ...
            {
                //await ProtoService.SendAsync(new ToggleMessageSenderIsBlocked(
                //    new MessageSenderUser(user.Id)/*, false*/));
                StartExecute();
            }
            else
            {
                var confirm = await MessagePopup.ShowAsync(
                    Strings.Resources.AreYouSureUnblockContact, 
                    Strings.Resources.AppName, Strings.Resources.OK, Strings.Resources.Cancel);
                if (confirm != ContentDialogResult.Primary)
                {
                    return;
                }
        ...
        private async void ReportExecute()
        {
            ...
            var response = await ProtoService.SendAsync(
                new ReportChat(chat.Id, new long[0], /*reason*/default, text));
        }
        ...
        private async void SetTimerExecute()
        {
            ...
            //RnD
            dialog.Value = default;//chat.MessageTtl;
    ...
    public class MessageCollection : MvxObservableCollection<MessageViewModel>
    {
        ...
        private static MessageViewModel ShouldUpdateSeparatorOnInsert(MessageViewModel item, MessageViewModel previous, int index)
        {
            if (item != null && previous != null)
            {
                ...
                {
                    //RnD
                    var service = new MessageViewModel(previous.ProtoService, previous.PlaybackService, 
                        previous.Delegate, 
                        default/*new Message(0, previous.SenderId, previous.ChatId, null, previous.SchedulingState, previous.IsOutgoing, false, false, false, false, true, false, false, false, false, false, false, false, previous.IsChannelPost, false, previous.Date, 0, null, null, null, 0, 0, 0, 0, 0, 0, string.Empty, 0, string.Empty, new MessageHeaderDate(), null)*/);
                    return service;
                }
            }
