        ...
        public IAsyncOperation<LoadMoreItemsResult> LoadMoreItemsAsync(uint count)
        {
            return AsyncInfo.Run(async token =>
            {
                ...
                if (_first && _settings.Stickers.SuggestionMode != StickersSuggestionMode.None)
                {
                    ...
                    var response = await _protoService.SendAsync(/*new GetStickers(_query, 1000)*/default);
                ...
                else if (!_first && _settings.Stickers.SuggestionMode == StickersSuggestionMode.All)
                {
                    ...
                    var response = await _protoService.SendAsync(/*new SearchStickers(_query, 20)*/default);
                    if (response is Stickers stickers)
                    {
                        foreach (var sticker in stickers.StickersValue)
                        {
