...
namespace Unigram.Services
{
        ...
        IHandle<UpdateActiveNotifications>
    {
        ...
        public async void Handle(UpdateTermsOfService update)
        {
            ...
            if (terms.ShowPopup)
            {
                async void DeleteAccount()
                {
                    ...
                    _protoService.Send(/*new DeleteAccount("Decline ToS update")*/default);
                }
        ...
        private async void ProcessNewPushMessage(int groupId, int id, long chatId, 
            NotificationTypeNewPushMessage message, int date, bool silent)
        {
            var chat = _protoService.GetChat(chatId) as Chat;
            if (chat == null)
            {
                return;
            }
            ...
            {
                await UpdateToast(caption, content, $"{_sessionService.Id}", sound, launch, $"{id}", 
                    $"{groupId}", picture, dateTime, canReply);
            });
        }

        private async void ProcessNewMessage(int groupId, int id, Message message, int date, bool silent)
        {
            var chat = _protoService.GetChat(message.ChatId) as Chat;
            if (chat == null)
            {
                return;
            }
        ...
        public async Task ProcessAsync(Dictionary<string, string> data)
        {
            ...
            if (data.TryGetValue("action", out string action))
            {
                ...
                if (string.Equals(action, "reply", StringComparison.OrdinalIgnoreCase) && data.TryGetValue("input", out string text))
                {
                    ...
                    var replyToMsgId = data.ContainsKey("msg_id") 
                        ? long.Parse(data["msg_id"]) << 20 : 0;
                    var response = await _protoService.SendAsync
                        (new SendMessage(chat.Id, 0, /*replyToMsgId*/default, 
                        new MessageSendOptions(false, true, false, default, default,default,default), 
                        null, new InputMessageText(formatted, default, false)));

                    if (chat.Type is ChatTypePrivate && chat.LastMessage != null)
                    {
                        await _protoService.SendAsync(
                            new ViewMessages(chat.Id, /*0*/default, 
                            /*new long[] { chat.LastMessage.Id }*/default, true));
                    }
                }
                else if (string.Equals(action, "markasread", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    await _protoService.SendAsync(new ViewMessages(chat.Id, default, 
                        /*new long[] { chat.LastMessage.Id }*/default, true));
                }
            }
        }
        ...
        private string UpdateFromLabel(Chat chat, Message message)
        {
            ...
            if (ShowFrom(_protoService, chat, message, out User from))
            {
                ...
                else if (!string.IsNullOrEmpty(from.Id.ToString()))
                {
                    result = $"{from.Id.ToString().Trim()}: ";
                }
        ...
        private string UpdateFromLabel(Chat chat, NotificationTypeNewPushMessage message)
        {
            ...
            if (_protoService.TryGetUser(message.SenderId, out User senderUser))
            {
                if (ShowFrom(chat))
                {
                    ...
                    else if (!string.IsNullOrEmpty(senderUser.Id.ToString()))
                    {
                        result = $"{senderUser.Id.ToString().Trim()}: ";
                    }
        ...
        public void SetMuteFor(Chat chat, int muteFor)
        {
            ...
            _protoService.Send(new SetChatNotificationSettings(chat.Id,
                new ChatNotificationSettings(
                    useDefault, muteFor,
                    settings.UseDefaultSound, default,
                    /*settings.Sound,*/
                    settings.UseDefaultShowPreview, settings.ShowPreview, default, default, default,
