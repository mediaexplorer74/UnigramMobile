...
namespace Unigram.Services
{
    public interface IProtoService : ICacheService
    {
        ...
        object GetChat(object senderChatId);
    ...
    public partial class ProtoService : IProtoService, ClientResultHandler
    {
            ...
            if (_settings.VersionLastStart < SettingsService.CurrentVersion)
            {
                ...
                if (response is Chat chat)
                {
                    ...
                    foreach (var entity in formattedText.Entities)
                    {
                        ...
                        {
                            //RnD
                            await SendAsync(new GetWebPagePreview(formattedText,default));
                            break;
                        }
                    }

                    Send(new AddLocalMessage(chat.Id, new MessageSenderUser(777000), default, 
                        false, new InputMessageText(formattedText, default, false)));
                }
            }
        ...
        private async void UpdateLanguagePackStrings(UpdateLanguagePackStrings update)
        {
            ...
            if (response is Chat chat)
            {
                ...
                var formattedText = new FormattedText(message, new[] 
                { new TextEntity 
                { 
                    Offset = 0, Length = title.Length, Type = new TextEntityTypeBold()
                } 
                });

                Send(new AddLocalMessage(chat.Id, new MessageSenderUser(777000), default, false, 
                    new InputMessageText(formattedText, default, false)));
            }
        }
        ...
        public object GetChat(object senderChatId)
        {
