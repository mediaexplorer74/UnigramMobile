        ...
        private async Task SendRatingAsync(int callId)
        {
            ...
            if (confirm == ContentDialogResult.Primary)
            {
                ...
                if (dialog.IncludeDebugLogs && dialog.Rating <= 3)
                {
                    ...
                    ProtoService.Send(new SendMessage(chat.Id, 0, /*0*/default,
                        new MessageSendOptions(false, false, false, default,null,default,default), 
                        null, new InputMessageDocument(new InputFileLocal(file.Path), null, false, null)));
                }
            }
        }
