...
namespace Unigram.Controls.Messages
{
    public class MessageService : SimpleButton
    {
        ...
        private static (string Text, IList<TextEntity> Entities) UpdateLocationChanged(
            MessageViewModel message, ChatEventLocationChanged locationChanged, bool active)
        {
            var content = string.Empty;
            var entities = active ? new List<TextEntity>() : null;
            ...
            {
                content = ReplaceWithLink(string.Format(Strings.Resources.EventLogChangedLocation, 
                    locationChanged.NewLocation.Address), "un1", fromUser, ref entities);
            }
            else
            {
                content = ReplaceWithLink(Strings.Resources.EventLogRemovedLocation, "un1", 
                    fromUser, ref entities);
            }
        ...
        private static (string Text, IList<TextEntity> Entities) UpdateMessageUnpinned(MessageViewModel message, ChatEventMessageUnpinned messageUnpinned, bool active)
        {
            ...
            content = ReplaceWithLink(Strings.Resources.EventLogUnpinnedMessages, "un1", 
                fromUser, ref entities);

            return (content, entities);
        }

        private static (string Text, IList<TextEntity> Entities) UpdateMessageDeleted(
            MessageViewModel message, ChatEventMessageDeleted messageDeleted, bool active)
        {
            var content = string.Empty;
            var entities = active ? new List<TextEntity>() : null;
            ...
            content = ReplaceWithLink(Strings.Resources.EventLogDeletedMessages,
                "un1", fromUser, ref entities);

            return (content, entities);
        }

        private static (string Text, IList<TextEntity> Entities) UpdateMessageEdited(
            MessageViewModel message, ChatEventMessageEdited messageEdited, bool active)
        {
            var content = string.Empty;
            var entities = active ? new List<TextEntity>() : null;
            ...
            {
                content = ReplaceWithLink(Strings.Resources.EventLogEditedMessages, "un1",
                    fromUser, ref entities);
            }
            else
            {
                content = ReplaceWithLink(Strings.Resources.EventLogEditedCaption, "un1", 
                    fromUser, ref entities);
            }
        ...
        private static (string Text, IList<TextEntity> Entities) UpdateMessageTtlChanged(
            MessageViewModel message, ChatEventMessageTtlChanged messageTtlChanged, bool active)
        {
            var content = string.Empty;
            var entities = active ? new List<TextEntity>() : null;
            ...
            {
                content = ReplaceWithLink(string.Format(Strings.Resources.ActionTTLChanged, 
                    Locale.FormatTtl(messageTtlChanged.NewMessageTtl)), "un1", fromUser, ref entities);
            }
            else
            {
        ...
        private static (string, IList<TextEntity>) UpdateChatSetTtl(MessageViewModel message,
            MessageChatSetTtl chatSetTtl, bool active)
        {
            var content = string.Empty;
            var entities = active ? new List<TextEntity>() : null;
            ...
            if (chat.Type is ChatTypeSecret)
            {
                if (chatSetTtl.Ttl != 0)
                {
                    if (message.IsOutgoing)
                    {
                        content = string.Format(Strings.Resources.MessageLifetimeChangedOutgoing,
                            Locale.FormatTtl(chatSetTtl.Ttl));
                    }
                    else
                    {
                        content = ReplaceWithLink(string.Format(Strings.Resources.MessageLifetimeChanged, 
                            "un1", Locale.FormatTtl(chatSetTtl.Ttl)), "un1", message.GetSender(), ref entities);
                    }
                }
                else
                {
        ...
        private static (string Text, IList<TextEntity> Entities) UpdatePassportDataSent(MessageViewModel message, MessagePassportDataSent passportDataSent, bool active)
        {
            ...
            var chat = message.ProtoService.GetChat(message.ChatId) as Chat;
            content = string.Format(Strings.Resources.ActionBotDocuments, chat?.Title 
                ?? string.Empty, str.ToString());
        ...
        private static (string Text, IList<TextEntity> Entities) UpdateWebsiteConnected(
            MessageViewModel message, MessageWebsiteConnected websiteConnected, bool active)
        {
