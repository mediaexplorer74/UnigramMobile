...
namespace Unigram.Controls.Messages
{
    public sealed partial class MessageBubble : StackPanel
    {
        ...
        public void UpdateMessageHeader(MessageViewModel message)
        {
            ...
            if (viaBot != null && viaBot.Type is UserTypeBot && !string.IsNullOrEmpty(viaBot.FirstName))
            {
                var hyperlink = new Hyperlink();
                hyperlink.Inlines.Add(new Run 
                { Text = paragraph.Inlines.Count > 0 
                    ? " via @" : "via @", FontWeight = FontWeights.Normal });
                hyperlink.Inlines.Add(new Run { Text = viaBot.FirstName });
                hyperlink.UnderlineStyle = UnderlineStyle.None;
                hyperlink.Foreground = light ? new SolidColorBrush(Colors.White) 
                    : GetBrush("MessageHeaderForegroundBrush");
        ...
        public void UpdateMessageContent(MessageViewModel message, bool padding = false)
        {
            ...
            else
            {
                ...
                {
                    //if ((sticker.Sticker.Type is StickerTypeAnimated || sticker.Sticker.Type is StickerTypeVideo))
                    //{
