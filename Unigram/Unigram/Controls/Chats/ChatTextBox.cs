    ...
    public class ChatTextBox : FormattedTextBox
    {
        ...
        protected override void OnKeyDown(KeyRoutedEventArgs e)
        {
            ...
            else if (e.Key == VirtualKey.Up || e.Key == VirtualKey.Down)
            {
                ...
                || (ViewModel.Autocomplete is SearchStickersCollection && Autocomplete.SelectedItem != null) || (ViewModel.Autocomplete is SearchStickersCollection == false && ViewModel.Autocomplete is EmojiCollection == false)))
            {
                var shift = Window.Current.CoreWindow.GetKeyState(VirtualKey.Shift).HasFlag(CoreVirtualKeyStates.Down);
                if (shift)
                {
        ...
        private async void OnSelectionChanged(object sender, RoutedEventArgs e)
        {
            if (_isMenuExpanded)
            {
                if (ViewModel.Autocomplete is AutocompleteList<UserCommand> == false)
                {
        ...
        public async Task SendAsync(bool disableNotification = false)
        {
            if (ViewModel.Type == DialogType.ScheduledMessages 
                && ViewModel.ComposerHeader?.EditingMessage == null)
            {
                await ScheduleAsync();
                return;
            }
            ...
            var options = new MessageSendOptions(disableNotification, false, 
                false, default,null,default,default);
        ...
        private bool SearchInlineBotResults(string text, out string searchText)
        {
            ...
            {
                var username = ViewModel.CurrentInlineBot.FirstName;
                if (text != null && text.TrimStart().StartsWith("@" + username, 
                    StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (!flag)
                    {
                        if (string.Equals(text.TrimStart(), "@" + username, 
                            StringComparison.OrdinalIgnoreCase))
                        {
                            ClearInlineBotResults();
                        }
        ...
        private void UpdateInlinePlaceholder(string username, string placeholder)
        {
            if (InlinePlaceholderTextContentPresenter != null)
            {
                ...
                {
                    InlinePlaceholderTextContentPresenter.Inlines.Add(new Run 
                    { Text = "@" + username + " ", Foreground = null });
                    InlinePlaceholderTextContentPresenter.Inlines.Add(
                        new Run { Text = placeholder });
                }
            }
        }
