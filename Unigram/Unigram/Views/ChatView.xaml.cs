        ...
        private void TryGetWebPagePreview(IProtoService protoService, Chat chat, string text, Action<BaseObject> result)
        {
            ...
            if (chat.Type is ChatTypeSecret)
            {
                ...
                if (response is TextEntities entities)
                {
                    ...
                    //protoService.Send(
                    //    new GetWebPagePreview(new FormattedText(urls, new TextEntity[0])), result);
                }
                else
                {
                    result(null);
        ...
        private void Action_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (message.IsSaved())
            {
                if (message.ForwardInfo?.Origin is MessageForwardOriginUser 
                    || message.ForwardInfo?.Origin is MessageForwardOriginChat)
                {
                    ViewModel.NavigationService.NavigateToChat(message.ForwardInfo.FromChatId, 
                        message.ForwardInfo.FromMessageId);
                }
                else if (message.ForwardInfo?.Origin is MessageForwardOriginChannel fromChannel)
                {
                    ViewModel.NavigationService.NavigateToChat(fromChannel.ChatId, 
                        fromChannel.MessageId);
                }
            }
            else
            {
...
#region Context menu
        ...
        private void Menu_ContextRequested(object sender, RoutedEventArgs e)
        {
            ...
            var basicGroup = chat.Type is ChatTypeBasicGroup basicGroupType 
                ? ViewModel.ProtoService.GetBasicGroup(basicGroupType.BasicGroupId) 
                : null;
            var supergroup = chat.Type is ChatTypeSupergroup supergroupType 
                ? ViewModel.ProtoService.GetSupergroup(supergroupType.SupergroupId) 
                : null;

            flyout.CreateFlyoutItem(ViewModel.SearchCommand, Strings.Resources.Search, 
                new FontIcon { Glyph = Icons.Search }, Windows.System.VirtualKey.F);

            if (supergroup != null && !(supergroup.Status is ChatMemberStatusCreator) 
                && (supergroup.IsChannel || !string.IsNullOrEmpty(supergroup.Id.ToString())))
            {
                flyout.CreateFlyoutItem(ViewModel.ReportCommand, Strings.Resources.ReportChat, 
                    new FontIcon { Glyph = Icons.Report });
            }
            ...
            if (user != null || (basicGroup != null && basicGroup.CanDeleteMessages())
                || (supergroup != null && supergroup.CanDeleteMessages()))
            {
                flyout.CreateFlyoutItem(ViewModel.SetTimerCommand, Strings.Resources.SetTimer, 
                    new FontIcon { Glyph = Icons.Timer });
            }
            if (user != null || basicGroup != null || 
                (supergroup != null && !supergroup.IsChannel 
                && string.IsNullOrEmpty(supergroup.Id.ToString())))
            {
                flyout.CreateFlyoutItem(ViewModel.ChatClearCommand, Strings.Resources.ClearHistory, 
                    new FontIcon { Glyph = Icons.Clear });
            }
            if (user != null)
            {
                flyout.CreateFlyoutItem(ViewModel.ChatDeleteCommand, Strings.Resources.DeleteChatUser,
                    new FontIcon { Glyph = Icons.Delete });
            }
            if (basicGroup != null)
            {
                flyout.CreateFlyoutItem(ViewModel.ChatDeleteCommand, Strings.Resources.DeleteAndExit, 
                    new FontIcon { Glyph = Icons.Delete });
            }
            if ((user != null && user.Id != ViewModel.CacheService.Options.MyId) || basicGroup != null 
                || (supergroup != null && !supergroup.IsChannel))
            {
            ...
            {
                UserFullInfo fullInfo = ViewModel.ProtoService.GetUserFull(user.Id) as UserFullInfo;
                if (fullInfo != null)
                {
        ...
        private async void Message_ContextRequested(UIElement sender, ContextRequestedEventArgs args)
        {
            ...
            else
            {
                ...
                // Stickers
                flyout.CreateFlyoutItem(MessageAddSticker_Loaded, ViewModel.MessageAddStickerCommand, message, Strings.Resources.AddToStickers, new FontIcon { Glyph = Icons.Stickers, FontFamily = Constants.SymbolThemeFontFamily });
        ...
        private bool MessageReply_Loaded(MessageViewModel message)
        {
            ...
            if (chat != null && chat.Type is ChatTypeSupergroup supergroupType)
            {
                ...
                {
                    return default;//restricted.Permissions.CanSendMessages;
                }
            }
        ...
        private bool MessagePin_Loaded(MessageViewModel message)
        {
            ...
            if (chat != null && chat.Type is ChatTypeSupergroup supergroupType)
            {
                ...
                if (supergroup.Status is ChatMemberStatusCreator 
                    || (supergroup.Status is ChatMemberStatusAdministrator admin 
                    /*&& (admin.CanPinMessages || supergroup.IsChannel && admin.CanEditMessages)*/))
                {
                    return true;
                }
                else if (supergroup.Status is ChatMemberStatusRestricted restricted)
            ...
            else if (chat != null && chat.Type is ChatTypeBasicGroup basicGroupType)
            {
                ...
                if ( basicGroup.Status is ChatMemberStatusCreator
                    /*|| (basicGroup.Status is ChatMemberStatusAdministrator admin && admin.CanPinMessages)*/)
                {
                    return true;
                }
            }
        ...
        private void Autocomplete_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            if (e.ClickedItem is User user && ChatTextBox.SearchByUsername(text.Substring(0, Math.Min(TextField.Document.Selection.EndPosition, text.Length)), out string username, out int index))
            {
                ...
                {
                    insert = user.FirstName;
                }

                var range = TextField.Document.GetRange(
                    TextField.Document.Selection.StartPosition - username.Length - adjust, 
                    TextField.Document.Selection.StartPosition);
                ...
                TextField.Document.GetRange(
                    range.EndPosition, range.EndPosition).SetText(TextSetOptions.None, " ");
                TextField.Document.Selection.StartPosition = range.EndPosition + 1;

                if (index == 0 && user.Type is UserTypeBot bot && bot.IsInline)
                {
                    ViewModel.ResolveInlineBot(user.FirstName);
                }
            }
            else if (e.ClickedItem is UserCommand command)
            {
                var input = text.Substring(0, Math.Min(TextField.Document.Selection.EndPosition, 
                    text.Length));
                ...
                if (ChatTextBox.SearchByCommand(input, out string initialCommand))
                {
                    ...
                    if (chat.Type is ChatTypeSupergroup || chat.Type is ChatTypeBasicGroup)
                    {
                        ...
                        if (bot != null && bot.FirstName.Length > 0)
                        {
                            insert += $"@{bot.FirstName}";
                        }
                    }
            ...
            else if (e.ClickedItem is string hashtag && ChatTextBox.SearchByHashtag(
                text.Substring(0, Math.Min(TextField.Document.Selection.EndPosition, text.Length)), out string initial, out int index2))
            {
                var insert = $"{hashtag} ";
                var start = TextField.Document.Selection.StartPosition - 1 - initial.Length 
                    + insert.Length;
                var range = TextField.Document.GetRange(TextField.Document.Selection.StartPosition
                    - 1 - initial.Length, TextField.Document.Selection.StartPosition);
                range.SetText(TextSetOptions.None, insert);
                ...
                && ChatTextBox.SearchByEmoji(text.Substring(0, Math.Min(
                    TextField.Document.Selection.EndPosition, text.Length)), out string replacement, out _))
            {
                var insert = $"{emoji.Value} ";
                var start = TextField.Document.Selection.StartPosition - 1 - replacement.Length
                    + insert.Length;
                var range = TextField.Document.GetRange(TextField.Document.Selection.StartPosition
                    - 1 - replacement.Length, TextField.Document.Selection.StartPosition);
                range.SetText(TextSetOptions.None, insert);
...
#region Binding
        ...
        private void Autocomplete_ContainerContentChanging(ListViewBase sender, ContainerContentChangingEventArgs args)
        {
            ...
            else if (args.Item is User user)
            {
                ...
                username.Text = string.IsNullOrEmpty(user.FirstName) 
                    ? string.Empty 
                    : $" @{user.FirstName}";
...
#region UI delegate
        ...
        public void UpdateChatNotificationSettings(Chat chat)
        {
            if (chat.Type is ChatTypeSupergroup super && super.IsChannel)
            {
                ...
                if (group.Status is ChatMemberStatusCreator 
                    /*|| group.Status is ChatMemberStatusAdministrator administrator
            ...
            else
            {
                string GetPlaceholder()
                {
                    if (ViewModel.CacheService.TryGetSupergroup(chat, out Supergroup supergroup))
                    {
                        ...
                        else if (supergroup.Status is ChatMemberStatusCreator creator && creator.IsAnonymous
                            || supergroup.Status is ChatMemberStatusAdministrator administrator /*&& administrator.IsAnonymous*/)
                        {
        ...
        public void UpdateComposerHeader(Chat chat, MessageComposerHeader header)
        {
            if (header == null)
            {
                ...
                bool rights = default;//ViewModel.VerifyRights(chat, x => x.CanSendMediaMessages,
                                      //Strings.Resources.GlobalAttachMediaRestricted, Strings.Resources.AttachMediaRestrictedForever,
                                      //Strings.Resources.AttachMediaRestricted, out string label);
                bool pollsRights = ViewModel.VerifyRights(chat, x => x.CanSendPolls,
                    Strings.Resources.GlobalAttachMediaRestricted, Strings.Resources.AttachMediaRestrictedForever,
                    Strings.Resources.AttachMediaRestricted, out string pollsLabel);

                bool pollsAllowed = chat.Type is ChatTypeSupergroup || chat.Type is ChatTypeBasicGroup;
            ...
            else
            {
                ...
                if (editing != null)
                {
                    switch (editing.Content)
                    {
                        ...
                        case MessagePhoto photo:
                            ButtonAttach.Glyph = !photo.IsSecret 
                                ? ReplyInfoToGlyphConverter.AttachEditGlyph
                                : ReplyInfoToGlyphConverter.AttachGlyph;

                            ButtonAttach.IsEnabled = !photo.IsSecret;
                            break;
                        case MessageVideo video:
                            ButtonAttach.Glyph = !video.IsSecret
                                ? ReplyInfoToGlyphConverter.AttachEditGlyph
                                : ReplyInfoToGlyphConverter.AttachGlyph;
                    ...
                    AttachCurrent.Visibility = editing.Content is MessagePhoto 
                        || editing.Content is MessageVideo 
                          ? Visibility.Visible 
                          : Visibility.Collapsed;
                ...
                else
                {
                    ...
                    bool rights = default; //ViewModel.VerifyRights(chat, x => x.CanSendMediaMessages, Strings.Resources.GlobalAttachMediaRestricted, Strings.Resources.AttachMediaRestrictedForever, Strings.Resources.AttachMediaRestricted, out string label);
                    var pollsRights = ViewModel.VerifyRights(chat, x => x.CanSendPolls,
                        Strings.Resources.GlobalAttachMediaRestricted, 
                        Strings.Resources.AttachMediaRestrictedForever, 
                        Strings.Resources.AttachMediaRestricted, out string pollsLabel);
                    ...
                    AttachDocument.Visibility = rights ? Visibility.Collapsed : Visibility.Visible;
                    AttachLocation.Visibility = Visibility.Visible;
                    AttachPoll.Visibility = (chat.Type is ChatTypeSupergroup
                        || chat.Type is ChatTypeBasicGroup) && !pollsRights 
        ...
        public void UpdateUserFullInfo(Chat chat, User user, UserFullInfo fullInfo, 
            bool secret, bool accessToken)
        {
            ...
            {
                ShowAction(ViewModel.CacheService.GetNotificationSettingsMuteFor(chat) > 0 
                    ? Strings.Resources.ChannelUnmute : Strings.Resources.ChannelMute, true);
            }
        ...
        public async void UpdateSupergroup(Chat chat, Supergroup group)
        {
            ...
            {
                if ((group.Status is ChatMemberStatusLeft && (group.Id.ToString().Length > 0 
                    || ViewModel.CacheService.IsChatAccessible(chat))) 
                    || (group.Status is ChatMemberStatusCreator creator && !creator.IsMember))
                {
                    ShowAction(Strings.Resources.ChannelJoin, true);
                }
                else if (group.Status is ChatMemberStatusCreator 
                    /*|| group.Status is ChatMemberStatusAdministrator administrator 
                    && administrator.CanPostMessages*/)
                {
                    ShowArea();
            ...
            {
                if ((group.Status is ChatMemberStatusLeft && (group.Id.ToString().Length > 0 
                    || group.HasLocation || group.HasLinkedChat 
                    || ViewModel.CacheService.IsChatAccessible(chat)))
                    || (group.Status is ChatMemberStatusCreator creator && !creator.IsMember))
                {
                    if (ViewModel.Type == DialogType.Thread)
                ...
                {
                    if (!restrictedSend.IsMember && group.Id.ToString().Length > 0)
                    {
                        ShowAction(Strings.Resources.ChannelJoin, true);
                    }
            ...
            else if (group.Status is ChatMemberStatusCreator creator
                && creator.IsAnonymous || group.Status is ChatMemberStatusAdministrator administrator 
                /*&& administrator.IsAnonymous*/)
            {
            ...
            if (ViewModel.Type == DialogType.History)
            {
                ViewModel.LastSeen = Locale.Declension(group.IsChannel 
                    ? "Subscribers" : "Members", group.MemberCount);
            }
            else
            {
                ViewModel.LastSeen = null;
        ...
        public void UpdateSupergroupFullInfo(Chat chat, Supergroup group, SupergroupFullInfo fullInfo)
        {
            if (ViewModel.Type == DialogType.History)
            {
                ViewModel.LastSeen = Locale.Declension(group.IsChannel ? "Subscribers" : "Members", 
        ...
        public void UpdateFile(File file)
        {
            if (_viewModel.TryGetMessagesForFileId(file.Id, out IList<MessageViewModel> messages))
            {
                foreach (var message in messages)
                {
                    ...
                    else if (message.Content is MessageChatChangePhoto
                    ...
                    if (content is MessageAnimation animation 
                        && animation.Animation.AnimationValue.Id == file.Id 
                        && file.Local.IsDownloadingCompleted)
                    {
                        ViewVisibleMessages(false);
                    }
                    else if (content is MessageVideoNote videoNote 
                        && videoNote.VideoNote.Video.Id == file.Id 
                        && file.Local.IsDownloadingCompleted)
                    {
                        ViewVisibleMessages(false);
                    }
                    else if (content is MessageSticker sticker
                        /*&& (sticker.Sticker.Type is StickerTypeAnimated 
                        || sticker.Sticker.Type is StickerTypeVideo)*/ 
                        && sticker.Sticker.StickerValue.Id == file.Id 
                        && file.Local.IsDownloadingCompleted)
                    {
                        ViewVisibleMessages(false);
                    }
                    else if (content is MessageDice && message.IsAnimatedStickerDownloadCompleted())
                    {
                        ...
                        && file.Local.IsDownloadingCompleted)
                    {
                        if (text.WebPage.Animation?.AnimationValue.Id == file.Id 
                            || text.WebPage.VideoNote?.Video.Id == file.Id)
                        {
                            ViewVisibleMessages(false);
                        }
                        else if (text.WebPage.Sticker?.StickerValue.Id == file.Id 
                            /*&& (text.WebPage.Sticker.Type is StickerTypeAnimated 
                            || text.WebPage.Sticker.Type is StickerTypeVideo)*/ )
                        {
                            ViewVisibleMessages(false);
                        }
                    }
            ...
            if (file.Local.IsDownloadingCompleted 
                && _viewModel.TryGetMessagesForPhotoId(file.Id, out IList<MessageViewModel> photos))
            {
                    ...
                    if (content is MessageBubble == false)
                    {
                        ...
                        if (photo != null)
                        {
                            if (message.IsSaved())
                            {
                                ...
                                {
                                    Chat f_user = default;
                                    User user = message.ProtoService.GetUser(
                                        /*fromUser.SenderUserId*/f_user);
                                    if (user != null)
                                    {
                                ...
                                {
                                    Chat originChat = 
                                        message.ProtoService.GetChat(fromChat.SenderChatId) as Chat;
                                    if (originChat != null)
                                    {
                                ...
                                {
                                    Chat originChannel = message.ProtoService.GetChat(fromChannel.ChatId) 
                                        as Chat;
