...
namespace Unigram.Views.Popups
{
    public sealed partial class SharePopup : ContentPopup
    {
        ...
        public Task<ContentDialogResult> ShowAsync(Message message, bool withMyScore = false)
        {
            ...
            var chat = ViewModel.ProtoService.GetChat(message.ChatId) as Chat;
            if (chat != null && chat.Type is ChatTypeSupergroup super && super.IsChannel
                && ViewModel.ProtoService.GetSupergroup(super.SupergroupId)
                is Supergroup supergroup && supergroup.Id.ToString().Length > 0)
            {
                var link = $"{supergroup.Id.ToString()}/{message.Id}";
            ...
            else if (message.Content is MessageGame game)
            {
                ...
                if (viaBot != null && viaBot.Id.ToString().Length > 0)
                {
                    ViewModel.ShareLink = new Uri(MeUrlPrefixConverter.Convert(ViewModel.ProtoService,
                        $"{viaBot.FirstName}?game={game.Game.ShortName}"));
                ...
                else if (args.Phase == 1)
                {
                    ...
                    else if (result.Chat != null && result.Chat.Type is ChatTypeSupergroup super)
                    {
                        ...
                        if (result.IsPublic)
                        {
                            ...
                            {
                                subtitle.Text = string.Format("@{0}, {1}", supergroup.Id.ToString(), 
                                    Locale.Declension(supergroup.IsChannel ? "Subscribers" : "Members", supergroup.MemberCount));
                            }
                            else
                            {
                                subtitle.Text = $"@{supergroup.Id.ToString()}";
                            }
                        }
