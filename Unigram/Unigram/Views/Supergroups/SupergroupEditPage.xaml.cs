...
namespace Unigram.Views.Supergroups
{
    public sealed partial class SupergroupEditPage : HostedPage, ISupergroupEditDelegate
    {
        ...
        public void UpdateSupergroup(Chat chat, Supergroup group)
        {
            ...
            ChatHistory.Visibility = group.CanChangeInfo() && string.IsNullOrEmpty(group.Id.ToString()) 
                && !group.IsChannel ? Visibility.Visible : Visibility.Collapsed;
        ...
        public void UpdateSupergroupFullInfo(Chat chat, Supergroup group, SupergroupFullInfo fullInfo)
        {
            ...
            if (group.CanInviteUsers())
            {
                if (fullInfo.InviteLink == null && string.IsNullOrEmpty(group.Id.ToString() ))
                {
                    ...
                    ViewModel.ProtoService.Send(new CreateChatInviteLink(
                        chat.Id, string.Empty, 0, 0, false));
                }
                else if (string.IsNullOrEmpty(group.Id.ToString()))
                {
                ...
                {
                    InviteLink.Text = MeUrlPrefixConverter.Convert(ViewModel.CacheService, 
                        group.Id.ToString());
