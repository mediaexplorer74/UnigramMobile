...
namespace Unigram.Views
{
    public sealed partial class ChatsPage : ChatsListView
    {
        ...
        private async void Chat_ContextRequested(UIElement sender, ContextRequestedEventArgs args)
        {
            ...
            {
                flyout.CreateFlyoutItem(viewModel.FolderRemoveCommand, /*(chatListFilter.ChatFilterId, chat)*/default, 
                    Strings.Resources.FilterRemoveFrom, new FontIcon
            ...
            else
            {
                ...
                if (response != null && response.ChatListsValue.Count > 0)
                {
                    ...
                    {
                        Services.ChatFilterInfo filter = filters.FirstOrDefault(/*x => x.Id == chatList.ChatFilterId*/default);
                        if (filter != null)
                        {
                            item.CreateFlyoutItem(ViewModel.FolderAddCommand, (filter.Id, chat), filter.Title,
                                new FontIcon { Glyph = Icons.FromFilter(Icons.ParseFilter(filter.IconName)), 
                                    FontFamily = App.Current.Resources["TelegramThemeFontFamily"] as FontFamily });
                        }
                    }
                    ...
                    if (filters.Count < 10 && item.Items.Count > 0)
                    {
                        ...
                        item.CreateFlyoutItem(ViewModel.FolderCreateCommand, chat, Strings.Resources.CreateNewFilter, 
                            new FontIcon { Glyph = Icons.Add });
                    }
            ...
            flyout.CreateFlyoutItem(DialogNotify_Loaded, viewModel.ChatNotifyCommand, chat, muted
                ? Strings.Resources.UnmuteNotifications : Strings.Resources.MuteNotifications, new FontIcon 
                { Glyph = muted ? Icons.Unmute : Icons.Mute });
            flyout.CreateFlyoutItem(DialogMark_Loaded, viewModel.ChatMarkCommand, chat, chat.IsUnread()
                ? Strings.Resources.MarkAsRead : Strings.Resources.MarkAsUnread, new FontIcon 
                { Glyph = chat.IsUnread() ? Icons.MarkAsRead : Icons.MarkAsUnread, 
                    FontFamily = App.Current.Resources["TelegramThemeFontFamily"] as FontFamily });
            flyout.CreateFlyoutItem(DialogClear_Loaded, viewModel.ChatClearCommand, chat, Strings.Resources.ClearHistory, 
                new FontIcon { Glyph = Icons.Clear });
            flyout.CreateFlyoutItem(DialogDelete_Loaded, viewModel.ChatDeleteCommand, chat, DialogDelete_Text(chat), 
                new FontIcon { Glyph = Icons.Delete });

            if (AllowSelection && viewModel.SelectionMode != ListViewSelectionMode.Multiple)
            {
                flyout.CreateFlyoutSeparator();
                flyout.CreateFlyoutItem(viewModel.ChatSelectCommand, chat, Strings.Resources.lng_context_select_msg, 
                    new FontIcon { 
                    Glyph = Icons.Select });
            }
        ...
        public bool DialogClear_Loaded(Chat chat)
        {
            ...
            if (chat.Type is ChatTypeSupergroup super)
            {
                ...
                if (supergroup != null)
                {
                    return string.IsNullOrEmpty(supergroup.Id.ToString()/*.Username*/) && !super.IsChannel;
                }
            }
