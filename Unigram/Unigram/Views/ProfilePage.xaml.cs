...
namespace Unigram.Views
{
    public sealed partial class ProfilePage : HostedPage, IProfileDelegate, Navigation.INavigablePage
    {
        ...
        public void UpdateUser(Chat chat, User user, bool secret)
        {
            ...
            UserPhone.Visibility = string.IsNullOrEmpty(user.PhoneNumber)
                ? Visibility.Collapsed : Visibility.Visible;

            Username.Badge = $"{user.FirstName}";
            Username.Visibility = string.IsNullOrEmpty(user.FirstName) 
                ? Visibility.Collapsed : Visibility.Visible;

            DescriptionTitle.Text = user.Type is UserTypeBot
                ? Strings.Resources.DescriptionPlaceholder : Strings.Resources.UserBio;
        ...
        public void UpdateUserFullInfo(Chat chat, User user, UserFullInfo fullInfo, bool secret, bool accessToken)
        {
            if (user.Type is UserTypeBot)
            {
                GetEntities(/*fullInfo.ShareText*/default);
                DescriptionPanel.Visibility = default;//string.IsNullOrEmpty(fullInfo.ShareText)
                    //? Visibility.Collapsed : Visibility.Visible;
            }
            else
            {
                GetEntities(/*fullInfo.Bio*/default);
                DescriptionPanel.Visibility = default;//string.IsNullOrEmpty(fullInfo.Bio) ? Visibility.Collapsed : Visibility.Visible;
            }
        ...
        public void UpdateSecretChat(Chat chat, SecretChat secretChat)
        {
            ...
            {
                SecretLifetime.Badge = default;//chat.MessageTtl > 0 ? Locale.FormatTtl(chat.MessageTtl) : Strings.Resources.ShortMessageLifetimeForever;
        ...
        public void UpdateBasicGroup(Chat chat, BasicGroup group)
        {
            ...
            GroupInvite.Visibility = group.Status is ChatMemberStatusCreator 
                /* || (group.Status is ChatMemberStatusAdministrator administrator && administrator.CanInviteUsers)*/ || chat.Permissions.CanInviteUsers ? Visibility.Visible : Visibility.Collapsed;
        ...
        {
            Subtitle.Text = Locale.Declension(group.IsChannel ? "Subscribers" : "Members", 
                group.MemberCount);

            DescriptionTitle.Text = Strings.Resources.DescriptionPlaceholder;

            Automation.SetToolTip(Edit, group.IsChannel
                ? Strings.Resources.ManageChannelMenu 
                : Strings.Resources.ManageGroupMenu);

            Call.Visibility = Visibility.Collapsed;
            Edit.Visibility = group.Status is ChatMemberStatusCreator
                || group.Status is ChatMemberStatusAdministrator 
                ? Visibility.Visible : Visibility.Collapsed;
            Edit.Glyph = Icons.Edit;

            Verified.Visibility = group.IsVerified ? Visibility.Visible : Visibility.Collapsed;

            Username.Badge = $"{group.Id.ToString()}";
            Username.Visibility = string.IsNullOrEmpty(group.Id.ToString()) 
                ? Visibility.Collapsed : Visibility.Visible;

            Location.Visibility = group.HasLocation ? Visibility.Visible : Visibility.Collapsed;

            if (group.IsChannel && !(group.Status is ChatMemberStatusCreator) 
                && !(group.Status is ChatMemberStatusLeft) && !(group.Status is ChatMemberStatusBanned))
            {
                MiscPanel.Visibility = Visibility.Visible;
            ...
            GroupInvite.Visibility = !group.IsChannel
                && (group.Status is ChatMemberStatusCreator 
                /*|| (group.Status is ChatMemberStatusAdministrator administrator && administrator.CanInviteUsers)*/
                || chat.Permissions.CanInviteUsers) ? Visibility.Visible : Visibility.Collapsed;
            ...
            if (chat.Type is ChatTypePrivate || chat.Type is ChatTypeSecret)
            {
                ...
                else
                {
                    flyout.CreateFlyoutItem(ViewModel.ShareCommand, Strings.Resources.ShareContact, 
                        new FontIcon { Glyph = Icons.Share });
                }
            }
            //if (writeButton != null)
            //{
