...
#region Handle
        ...
        private void UpdateLeftTabsScale(bool show)
        {
            ...
            if (Window.Current.Content is RootPage root)
            {
                root.SetTopPadding(new Thickness(show ? 72 * SettingsService.Current.Appearance.TabsLeftLayoutScale : 0, 0, 0, 0));
            }
        }
...
#region Binding
                ...
                else if (args.Phase == 1)
                {
                    ...
                    else if (result.Chat != null && result.Chat.Type is ChatTypeSupergroup super)
                    {
                        ...
                        if (result.IsPublic)
                        {
                            ...
                            {
                                subtitle.Text = string.Format("@{0}, {1}", supergroup.Id /*Username*/, Locale.Declension(supergroup.IsChannel ? "Subscribers" : "Members", supergroup.MemberCount));
                            }
                            else
                            {
                                subtitle.Text = $"@{supergroup.Id/*.Username*/}";
                            }
                        }
                        else if (supergroup.MemberCount > 0)
                        {
                            subtitle.Text = Locale.Declension(supergroup.IsChannel 
                                ? "Subscribers" : "Members", supergroup.MemberCount);
                        }
                        else
                        {
                    ...
                    {
                        if (subtitle.Text.StartsWith($"@{result.Query}", 
        ...
        public async void NavigationView_ItemClick(RootDestination destination)
        {
            ...
            {
                MessageHelper.NavigateToUsername(ViewModel.ProtoService, MasterDetail.NavigationService, "UnigramMobileNews", null, null, null, null);
            }
        }
