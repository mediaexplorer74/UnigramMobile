...
namespace Telegram.Api.Services
{
    ...
    public partial class MTProtoService : ServiceBase, IMTProtoService, IDisposable
    {
        ...
        public MTProtoService(IDeviceInfoService deviceInfo,IUpdatesService updatesService, ICacheService cacheService, ITransportService transportService, IConnectionService connectionService)
        {
            ...
            {
                var transportDCId = _activeTransport != null
                    ? _activeTransport.DCId : Constants.FirstServerDCId;

                var transportKey = _activeTransport != null 
                    ? _activeTransport.AuthKey : null;

                var transportSalt = _activeTransport != null
                    ? _activeTransport.Salt : null;

                var transportSessionId = _activeTransport != null
                    ? _activeTransport.SessionId : null;

                var transportSequenceNumber = _activeTransport != null 
                    ? _activeTransport.SequenceNumber : 0;

                var transportClientTicksDelta = _activeTransport != null
                    ? _activeTransport.ClientTicksDelta : 0;
                
                _activeTransport = _transportService.GetTransport
                    (Constants.FirstServerIpAddress,
                    Constants.FirstServerPort, Type, out bool isCreated);
                    ...
                    foreach (var item in timedOutItems)
                    {
                        try
                        {
                            item.FaultCallback?.Invoke(
                                ...
                                {
                                    ErrorCode = (int)TLErrorCode.TIMEOUT,
                                    ErrorMessage = "MTProtoService: operation timed out ("
                                    + delayedTimeout + "s)"
                                });
#if DEBUG
                            TLUtils.WriteLine(item.Caption + " time out", LogSeverity.Error);
        ...
        public void GetConfigInformationAsync(Action<string> callback)
        {
            Execute.BeginOnThreadPool(() =>
            {
                ...
                sb.AppendLine(string.Format("{0} {1}", currentTime,
                    TLUtils.ToDateTime(currentTime).ToUniversalTime().ToString(
        ...
        public void UpdateTransportInfoAsync(int dcId, string dcIpAddress, 
            int dcPort, Action<bool> callback)
        {
            ...
            args.Update = new TLUpdateDCOptions{DCOptions 
                = new TLVector<TLDCOption>{ dcOption }};
