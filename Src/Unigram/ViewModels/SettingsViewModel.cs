...
namespace Unigram.ViewModels
{
   public class SettingsViewModel : UnigramViewModelBase
    {
        public SettingsViewModel(IMTProtoService protoService,
            ICacheService cacheService, ITelegramEventAggregator aggregator) 
            ...
            NavigationMode mode, IDictionary<string, object> state)
        {
            ...
            {
                var response = await ProtoService.GetUsersAsync(new TLVector<TLInputUserBase>
                { new TLInputUserSelf() });
        ...
        public RelayCommand DeleteAccountCommand
        {
            get
            {
                return new RelayCommand(DeleteAccountExecute);
            }
        }
        ...
        private async void DeleteAccountExecute()
        {
            ...
            if (Telegram.Api.Constants.FirstServerIpAddress.Equals("149.154.167.40"))
            {
                ...
                if (confirm == ContentDialogResult.Primary 
                    && dialog.Text.Equals(Self.Phone) && Self.Username != "frayxrulez")
                {
                    var really = await TLMessageDialog.ShowAsync("REAAAALLY???",
                        "REALLYYYY???",
                        "YES",
                        "NO I DON'T WANT TO");
